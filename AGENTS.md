...
**Current phase:** Phase 2 — Real-World Validation (Dogfooding). Phase 1 feature completeness achieved 2026-04-12. Repo is currently PRIVATE and will be relaunched as a new public repository with curated history. See [`STATUS.md`](STATUS.md) for the up-to-date phase, recently shipped PRs, test and build state, and blockers.
...
- **Godot binary:** `/home/hasnobeef/Apps/godot/sharppeak/Godot_v4.6.1-stable_mono_linux.x86_64` (per-project install on the dev machine). Wick's integration tests, once they exist, will launch a headless Godot instance.
- **`csharp-ls`:** `/home/hasnobeef/.dotnet/tools/csharp-ls` (global tool v0.22.0.0). Verified to work on .NET 10.
- **SDK:** `/home/hasnobeef/.dotnet/sdk/10.0.201/`.
- **Worktrees root:** `/home/hasnobeef/buildepicshit/Wick-worktrees/` (one subdir per active PR — see Worktree Workflow below).
...
5. **Worktree per PR.** Development happens in a sibling worktree under `/home/hasnobeef/buildepicshit/Wick-worktrees/`, never on `main` directly. See [Worktree Workflow](#worktree-workflow).
