...
> **Snapshot as of 2026-04-19.** This file answers *"where is Wick right now?"* — current phase, recent work, test and build state, blockers, next steps. Updated after significant project changes. Forward-looking roadmap: [`docs/planning/2026-04-11-roadmap-to-public-launch.md`](docs/planning/2026-04-11-roadmap-to-public-launch.md). Engineering standards: [`CONTRIBUTING.md`](CONTRIBUTING.md). Architecture: [`docs/architecture.md`](docs/architecture.md). Project overview: [`README.md`](README.md).

## What Wick is

Wick is a native .NET 10 Model Context Protocol server that captures unhandled C# exceptions from a running Godot game, enriches each exception with Roslyn-powered source context (calling method body, surrounding lines, caller chain), and exposes the enriched exception stream to AI coding assistants over MCP. The value proposition is narrow and deliberate: **when a Godot C# game crashes, the assistant sees the exception with full source-level context and can fix it in one turn instead of ten.**
...
2. **Discoverability fixes from the first pass** — `runtime_diagnose` description should explicitly route tool clients away from composing `runtime_status` + `runtime_get_exceptions` + `runtime_get_log_tail`; in-process bridge should be nudged when `runtime_status` sees a connected game without a `WICK_BRIDGE_TOKEN`. Both raised as P1/P2 findings in the bes-splash-3d pass.
